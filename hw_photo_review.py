...
def _run_ai_page_check_by_text(photo_url: str, page_start: int, page_end: int) -> dict:
    """사진 속에 인쇄된 페이지번호 '숫자'를 직접 읽어서 판단하는 기존 방식.
    ...
    [2026-08-13] 참조 PDF가 없는 항목에 대한 폴백(fallback)으로만 쓰인다 —
    참조 PDF가 있으면 hw_reference.run_ai_page_check_with_reference()의
    사진↔페이지 이미지 대조 방식이 우선 시도된다(더 정확함).
    """
...
def run_ai_page_check(
    ...
    *,
    ...
    """사진 속 페이지를 1차로 추정해서 항목 범위 안인지 확인하고 결과를
    hw_photos에 기록한다.

    [2026-08-13] class_id + material_name이 주어지고 그 반에 참조 PDF가
    등록돼 있으면(hw_reference.py), 손글씨 숫자를 읽는 대신 사진↔실제 페이지
    이미지 대조 방식을 먼저 시도한다 — 이 편이 훨씬 정확하다. 참조 자료가
    없으면(또는 오류가 나면) 기존 텍스트 인식 방식으로 자동 전환된다.
    ...
    if class_id is not None and material_name:
        try:
            from hw_reference import run_ai_page_check_with_reference

            result = run_ai_page_check_with_reference(
                photo_url, class_id, material_name, page_start, page_end
            )
        except Exception:
            # 참조 방식 자체가 실패해도(모듈/DB 오류 등) 기존 텍스트 인식으로
            # 넘어가야지, 검증 전체가 죽으면 안 된다.
            result = None

    if result is None:
        result = _run_ai_page_check_by_text(photo_url, page_start, page_end)

    conn = get_conn()
    conn.execute(
        "UPDATE hw_photos SET ai_page_guess = ?, ai_flag = ? WHERE id = ?",
        (result.get("guess"), result.get("flag"), photo_id),
    )
    conn.commit()
    conn.close()
    return result
...
def render_photo_review(
    ...
    [2026-08-13] class_id를 넘기면 "다시 확인" 버튼이 참조 PDF 대조 방식을
    먼저 시도한다(hw_reference.py). 안 넘기면(None) 기존 텍스트 인식 방식만
    쓴다 — 이전 호출부와 100% 호환된다.
    """
    item_states_df = get_items_with_state(assignment_id, submission_id)
                    ...
                    if st.button(btn_label, key=f"{btn_key_base}_ai", width="stretch"):
                        with st.spinner("사진 확인 중..."):
                            result = run_ai_page_check(
                                ...
                                class_id=class_id, material_name=irow["material_name"],
                            )
