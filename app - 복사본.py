...
def _safe_streamlit_dataframe(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in out.columns:
        ...
        if any(h in col_l for h in numeric_hints):
            ...
            if "count" in col_l or "오답" in col_l:
...
def init_db():
    ...
    cls_cols = [row[1] for row in c.execute("PRAGMA table_info(classes)").fetchall()]
...
def add_teacher(name: str) -> int:
    ...
    try:
        cur = conn.execute(
            "INSERT INTO teachers (name, created_at) VALUES (?, ?)",
...
def add_class(name: str, description: str, teacher_id: int | None = None):
    conn = get_conn()
    try:
        conn.execute(
...
def create_exam(
    ...
    conn = get_conn()
    cur = conn.execute(
        "INSERT INTO exams (name, exam_date, class_id, description) VALUES (?, ?, ?, ?)",
...
def _questions_df_from_db(test_id: int) -> pd.DataFrame:
    ...
    if not rows:
        return pd.DataFrame(columns=["문항번호", "단원", "난이도"])
    return pd.DataFrame(
        [
            {
                "문항번호": r["question_number"],
                "단원": r["topic"],
                "난이도": r["difficulty"],
            }
            for r in rows
...
def _build_questions_editor_df(parsed: dict | None) -> pd.DataFrame:
    ...
    for slot in _parsed_found_question_slots(parsed):
        ...
        if not topic:
            topic, _ = infer_topic_from_text(text, n)
        diff = str(slot.get("difficulty") or "Mid").strip() or "Mid"
        if diff not in ("High", "Mid", "Low"):
            diff = "Mid"
        rows.append({"문항번호": n, "단원": topic, "난이도": diff})
    if not rows:
        return pd.DataFrame(columns=["문항번호", "단원", "난이도"])
    return pd.DataFrame(rows).sort_values("문항번호").reset_index(drop=True)
...
def _render_test_question_editor(parsed: dict | None) -> bool:
    ...
    if (
        ...
        if st.button("TEST 다시 등록", key="ocr_reedit_test_btn"):
    ...
    with st.container(border=True):
        ...
        edited = st.data_editor(
            ...
            column_config={
                "문항번호": st.column_config.NumberColumn(
                    "문항번호", min_value=1, step=1
                ),
                "단원": st.column_config.TextColumn("단원", width="large"),
        ...
        if confirm_btn:
            if edited.empty:
                ...
                for _, row in edited.iterrows():
                    questions.append(
                        {
                            "question_number": int(row["문항번호"]),
                            "topic": str(row["단원"] or "미분류").strip() or "미분류",
                            "difficulty": str(row["난이도"] or "Mid").strip() or "Mid",
                        }
                    )
...
def _render_ocr_class_student_picker(
    *,
    ...
    """반·학생 선택 + 오답 체크 + DB 저장 (시험지 확정 후 표시)."""
    ...
    with st.container(border=True):
        ...
        if ocr_classes.empty:
            st.warning(
                "등록된 반이 없습니다. 앱을 재시작하면 **테스트 반**이 자동 생성됩니다."
            )
            st.session_state.pop("current_student_id", None)
            st.session_state.pop("current_student_name", None)
            return False

        class_map = {
            str(row["name"]): int(row["id"]) for _, row in ocr_classes.iterrows()
        }
        class_names = list(class_map.keys())

        prev_class_id = st.session_state.get("ocr_selected_class_id")
        if prev_class_id in class_map.values():
            default_class = next(
                n for n, cid in class_map.items() if cid == prev_class_id
            )
        else:
            default_class = (
                TEST_CLASS_NAME if TEST_CLASS_NAME in class_map else class_names[0]
            )
        ...
        st.session_state["ocr_selected_class_id"] = class_id

        students_in_class = get_students_by_class(class_id)
        if students_in_class.empty:
            st.warning(f"**{sel_class}** 반에 등록된 학생이 없습니다.")
            st.session_state.pop("current_student_id", None)
            st.session_state.pop("current_student_name", None)
            return False

        student_map: dict[str, int | None] = {"— 학생 선택 —": None}
        for _, row in students_in_class.iterrows():
            student_map[str(row["name"])] = int(row["id"])
        student_names = list(student_map.keys())

        prev_sid = st.session_state.get("current_student_id")
        if prev_sid in student_map.values():
            default_student = next(
                n for n, sid in student_map.items() if sid == prev_sid
            )
        else:
            default_student = "— 학생 선택 —"

        col_student, col_wrong = st.columns([1, 2], gap="large")
        ...
        with col_student:
            ...
            picked_sid = student_map[sel_student]
            if picked_sid:
                _bind_report_student(picked_sid, sel_student, class_id=class_id)

        with col_wrong:
            selected_wrong = _render_ocr_wrong_checkboxes(parsed)

        _sync_ocr_selected_collected(parsed, selected_wrong)

        st.divider()
        preview_score = _preview_score_from_wrong(selected_wrong)
        ...
        with meta1:
            if active:
                st.text_input(
                    ...
                    disabled=True,
                    key="ocr_exam_name_display",
                )
            else:
                st.text_input(
                    "테스트명",
                    placeholder="예: 3월 주간 테스트",
                    key="ocr_exam_name",
                )
        with meta2:
            if preview_score is not None:
                st.metric(
                    "예상 점수",
                    f"{preview_score:.1f}점",
                    delta=f"오답 {len(selected_wrong)}개",
                )
            else:
                st.caption("오답 선택 시 점수 자동 계산")
        with meta3:
            st.markdown("<div style='height:28px'></div>", unsafe_allow_html=True)
            save_wrong_btn = st.button(
                "오답 DB 저장",
                type="primary",
                width="stretch",
                key="ocr_save_wrong_btn",
            )

        if not picked_sid:
            st.warning("학생을 선택하면 오답 저장·PDF 생성을 진행할 수 있습니다.")
            return False

        if save_wrong_btn:
            _handle_ocr_test_result_save(
                student_id=picked_sid,
                student_name=sel_student,
                selected_wrong=selected_wrong,
            )

        st.success(
            f"선택됨: **{sel_class} · {sel_student}** (ID {picked_sid})",
        )
        return True
...
def _parent_report_preview_dialog(
    *, student_id: int, test_id: int, record: dict[str, Any]
    ...
    """Preview modal before generating parent report PDF."""
    ...
    test_avg = get_test_average_score(test_id)
    ...
    with opt1:
        include_avg = st.checkbox(
            "평균점수 포함", value=True, key="preview_include_avg"
        )
    with opt2:
        include_chart = st.checkbox(
            "누적 그래프 포함", value=True, key="preview_include_chart"
        )

    teacher_msg = st.text_area(
        "학부모 전달 메시지",
        placeholder="학부모님께 전달할 말씀을 입력해 주세요. PDF 「선생님이 드리고 싶은 말」 섹션에 반영됩니다.",
        key="preview_teacher_message",
        height=120,
    )
    ...
    preview_lines = [
        f"- **학생**: {student_name}",
        f"- **시험**: {record['test_name']} ({record['date']})",
        f"- **점수**: {record['score']:.1f}점",
        f"- **오답**: {', '.join(str(n) for n in (record.get('wrong_numbers') or [])) or '없음'}",
    ]
    if include_avg:
        if test_avg is not None:
            preview_lines.append(
                f"- **응시 전체 평균**: {test_avg:.1f}점 (차이 {record['score'] - test_avg:+.1f}점)"
            )
        else:
            preview_lines.append("- **응시 전체 평균**: (아직 응시 데이터 없음)")
    if include_chart:
        preview_lines.append(f"- **누적 그래프**: {len(history)}회 시험 기록 반영")
    if teacher_msg.strip():
        preview_lines.append(
            f"- **학부모 메시지**: {teacher_msg.strip()[:80]}{'…'if len(teacher_msg.strip()) > 80 else ''}"
        )
    st.markdown("\n".join(preview_lines))
    st.caption("표지 · 성적 요약 · 종합 총평 · 학습 처방 · 선생님 메시지가 포함됩니다.")

    btn_row1, btn_row2 = st.columns(2)
    with btn_row1:
        if st.button("PDF 생성 및 저장", type="primary", key="preview_confirm_pdf"):
            try:
                with st.spinner("학부모용 리포트 PDF 생성 중…"):
                    pdf_bytes, pdf_fname = generate_parent_report_pdf_from_db(
                        student_id,
                        test_id,
                        include_average=include_avg,
                        include_cumulative_chart=include_chart,
                        teacher_message=teacher_msg.strip(),
                    )
                st.session_state["ocr_parent_pdf_bytes"] = pdf_bytes
                st.session_state["ocr_parent_pdf_fname"] = pdf_fname
                st.success(
                    "PDF 생성 완료. 아래 버튼에서 저장 위치와 파일 이름을 지정해 주세요."
                )
                st.download_button(
                    "PDF 저장",
                    data=pdf_bytes,
                    ...
                    key="preview_download_pdf_btn",
                )
            except Exception as exc:
                st.error(f"생성 실패: {exc}")
    with btn_row2:
        if st.button("닫기", key="preview_close_btn"):
            st.rerun()
    ...
    with st.container(border=True):
        ...
        if extract_result:
            if extract_result.get("status") == "db_not_ready":
                ...
                debug_logs = extract_result.get("debug_logs") or []
                if debug_logs:
                    with st.expander("매칭 디버그 로그 (터미널에도 출력됨)"):
                        st.code("\n".join(debug_logs), language="text")
            elif extract_result.get("status") == "ok":
                st.success("유사문제 추출 완료")
                debug_logs = extract_result.get("debug_logs") or []
                if debug_logs:
                    with st.expander("매칭 디버그 로그 (터미널에도 출력됨)"):
                        st.code("\n".join(debug_logs), language="text")
        ...
        with st.container(border=True):
            ...
            with st.form("dashboard_new_student_form", clear_on_submit=True):
                ...
                with c1:
                    ...
                    school = st.text_input("학교", max_chars=80)
                    grade = st.text_input("학년", max_chars=20, placeholder="예: 중2, 고1")
                with c2:
                    pre_visit = st.text_area("내원 전 진도", height=100, placeholder="현재 학습 진도, 취약 단원 등")
                    contact = st.text_input("학생/학부모 연락처 *", max_chars=80, placeholder="010-0000-0000")
    ...
    with tab_edit:
        with st.container(border=True):
            ...
            if students_df.empty:
                ...
                if st.button("수정하기", key="dash_edit_open_btn"):
                ...
                if st.session_state.get("dash_edit_open") == sel_edit_id:
                    with st.form("dash_edit_student_form"):
                        new_name = st.text_input("이름", value=sel_row["name"])
                        new_phone = st.text_input("연락처", value=str(sel_row.get("parent_phone", "")))
                        new_class = st.selectbox("반", list(class_options.keys()),
                        ...
                        if st.form_submit_button("저장", type="primary"):
                            if _is_manager():
                                conn = get_conn()
                                conn.execute(
                                    "UPDATE students SET name=?, parent_phone=?, class_id=? WHERE id=?",
                                    (new_name.strip(), new_phone.strip(), class_options[new_class], sel_edit_id),
                                )
                                _commit(conn)
                                conn.close()
...
def _dashboard_class_manage() -> None:
    ...
    with st.container(border=True):
        ...
        with st.form("dash_add_class_form", clear_on_submit=True):
            class_name = st.text_input("수업 이름", placeholder="예: 중1 심화반", max_chars=60)

            # 수업 시간 설정
            st.markdown("**수업 시간**")
            if "dash_class_times" not in st.session_state:
                st.session_state["dash_class_times"] = [{"day": "월", "start": "16:00", "end": "18:00"}]

            DAY_OPTIONS = ["월", "화", "수", "목", "금", "토", "일"]
            time_data = []
            for i, slot in enumerate(st.session_state.get("dash_class_times", [{}])):
                c1, c2, c3 = st.columns([1, 1.2, 1.2])
                day = c1.selectbox("요일", DAY_OPTIONS, key=f"dash_day_{i}",
                                   index=DAY_OPTIONS.index(slot.get("day", "월")))
                start = c2.text_input("시작", value=slot.get("start", "16:00"), key=f"dash_start_{i}", placeholder="16:00")
                end = c3.text_input("종료", value=slot.get("end", "18:00"), key=f"dash_end_{i}", placeholder="18:00")
                time_data.append({"day": day, "start": start, "end": end})

            class_desc = st.text_area("설명 (선택)", max_chars=200, height=60)
            teacher_opts: dict[str, int | None] = {"— 담당 강사 미지정 —": None}
            for _, t in teachers_df.iterrows():
                teacher_opts[t["name"]] = int(t["id"])
            sel_teacher_label = st.selectbox("담당 강사", list(teacher_opts.keys()))

            submitted = st.form_submit_button("수업 생성", type="primary", use_container_width=True)

        col_add, _ = st.columns([1, 3])
        with col_add:
            if st.button("+ 시간 추가", key="dash_add_time"):
                st.session_state["dash_class_times"] = st.session_state.get("dash_class_times", []) + [{"day": "월", "start": "16:00", "end": "18:00"}]
                st.rerun()
            if len(st.session_state.get("dash_class_times", [])) > 1:
                if st.button("마지막 시간 삭제", key="dash_del_time"):
                    st.session_state["dash_class_times"] = st.session_state["dash_class_times"][:-1]
                    st.rerun()
        ...
        if submitted:
            if not class_name.strip():
                ...
                try:
                    ...
                    schedule_json = _json.dumps(time_data, ensure_ascii=False)
                    conn = get_conn()
                    conn.execute(
                        "INSERT INTO classes (name, description, teacher_id, schedule) VALUES (?, ?, ?, ?)",
                        (class_name.strip(), class_desc.strip(), teacher_opts[sel_teacher_label], schedule_json),
                    )
                    _commit(conn)
                    conn.close()
                    st.session_state["dash_class_times"] = [{"day": "월", "start": "16:00", "end": "18:00"}]
                    ...
                    st.error(f"같은 이름의 수업이 이미 존재합니다. ({e})")
    ...
    with col_left:
        with st.container(border=True):
            ...
            with st.form("add_class_form", clear_on_submit=True):
                ...
                class_desc = st.text_area(
                    "설명 (선택)",
                    placeholder="예: 매주 화·목 19:00, 중학교 1학년 대상",
                    max_chars=200,
                    height=80,
                )

                # Teacher dropdown — default to current sidebar selection.
                teacher_opts: dict[str, int | None] = {"— 담당 강사 미지정 —": None}
                for _, t in teachers_df.iterrows():
                    teacher_opts[t["name"]] = int(t["id"])
                default_idx = 0
                ...
                sel_teacher_label = st.selectbox(
                    "담당 강사",
                    list(teacher_opts.keys()),
                    index=default_idx,
                )

                if st.form_submit_button("수업 생성", width="stretch", type="primary"):
                    if not class_name.strip():
                        st.error("수업 이름을 입력해 주세요.")
                    else:
                        try:
                            add_class(
                                class_name, class_desc, teacher_opts[sel_teacher_label]
                            )
                            st.success(
                                f"수업 **{class_name.strip()}** 이(가) 생성되었습니다."
                            )
                            st.rerun()
                        except Exception:
                    ...
                    with st.container(border=True):
                        ...
                        with act_col:
                            with st.popover("강사 변경", use_container_width=True):
                                ...
                                if st.button(
                                    "저장",
                                    key=f"cls_t_save_{cls['id']}",
                                    type="primary",
                                    use_container_width=True,
                                ):
                                    assign_teacher_to_class(
                                        int(cls["id"]), t_opts2[new_t_label]
                                    )
                                    st.success("강사 배정이 변경되었습니다.")
                                    st.rerun()
                            if st.button(
                                "삭제",
                                key=f"del_cls_{cls['id']}",
                                use_container_width=True,
                            ):
                                delete_class(int(cls["id"]))
                                st.warning(
...
def _render_school_grade_tab() -> None:
    """School exam — math only; year / grade / semester / mid-final."""
    teacher_id = _current_teacher_id()
    students_df = get_all_students(teacher_id)
    ...
    with st.container(border=True):
        ...
        st.caption(
            f"**{MATH_SUBJECT}** 성적만 입력 · `student_grade_unified`에 누적 저장됩니다."
        )
        ...
        with c1:
            school_year = st.selectbox(
                "연도",
                year_options,
                index=year_options.index(date.today().year),
                key="school_grade_year",
            )
        with c2:
            grade_level = st.selectbox(
                "학년", GRADE_LEVEL_OPTIONS, key="school_grade_grade"
            )
        with c3:
            semester = st.selectbox(
                "학기", SEMESTER_OPTIONS, key="school_grade_semester"
            )
        with c4:
            exam_kind = st.selectbox(
                "시험 종류", SCHOOL_EXAM_KIND_OPTIONS, key="school_grade_kind"
            )
    ...
    with st.container(border=True):
        st.markdown("#### 수학 성적 입력")
        with st.form("school_math_grade_form"):
            sel_student = st.selectbox(
                "학생", list(student_opts.keys()), key="school_grade_student"
            )
            math_score = st.number_input(
                ...
                max_value=100.0,
                step=0.5,
                key="school_grade_score",
            )
            submitted = st.form_submit_button(
                "성적 저장", type="primary", use_container_width=True
            )
    ...
    with st.container(border=True):
        st.markdown("#### 저장된 성적")
        if records_df.empty:
            st.info("선택 조건에 저장된 성적이 없습니다.")
        else:
            view = records_df[
                ["student_name", "class_name", "score", "updated_at"]
            ].copy()
            view.columns = ["학생", "반", "수학 점수", "저장일"]
...
def _render_mock_grade_tab() -> None:
    """Mock exam — math only; year / month / grade."""
    teacher_id = _current_teacher_id()
    students_df = get_all_students(teacher_id)
    year_options = list(range(date.today().year - 2, date.today().year + 2))
    ...
    with st.container(border=True):
        ...
        st.caption(
            f"**{MATH_SUBJECT}** 성적만 입력 · 연도 / 월 / 학년 기준으로 저장됩니다."
        )
        ...
        with c3:
            grade_level = st.selectbox(
                "학년", MOCK_GRADE_LEVEL_OPTIONS, key="mock_grade_grade"
            )
    ...
    with st.container(border=True):
        st.markdown("#### 수학 성적 입력")
        with st.form("mock_math_grade_form"):
            sel_student = st.selectbox(
                "학생", list(student_opts.keys()), key="mock_grade_student"
            )
            math_score = st.number_input(
                f"{MATH_SUBJECT} 점수",
                min_value=0.0,
    ...
    with st.container(border=True):
        ...
        if records_df.empty:
            ...
            view = records_df[
                ["student_name", "class_name", "score", "exam_month", "updated_at"]
            ].copy()
            view.columns = ["학생", "반", "수학 점수", "월", "저장일"]
            st.dataframe(view, hide_index=True, use_container_width=True)
...
def _build_grade_trend_figure(df: pd.DataFrame) -> go.Figure | None:
    """Smoothed Plotly trend chart (screen + future PDF export)."""
    if df.empty:
        return None
    ...
    for group_key, label in UNIFIED_GROUP_LABELS.items():
        ...
        if plot_df.empty:
            continue
        has_trace = True
        color = _GRADE_TREND_COLORS[group_key]
        fig.add_trace(
            go.Scatter(
                ...
                hovertemplate=(
                    "%{x|%Y-%m-%d}<br>점수: %{y:.1f}점<extra>" + label + "</extra>"
                ),
            )
        )
    ...
    _axis_title_font = dict(size=13, color="#475569")
    _axis_tick_font = dict(size=12, color="#64748b")

    fig.update_layout(
        title=None,
        height=420,
        margin=dict(l=52, r=28, t=20, b=52),
        plot_bgcolor="#ffffff",
        paper_bgcolor="#ffffff",
        xaxis=dict(
            title=dict(text="시험 날짜", font=_axis_title_font),
            showgrid=True,
            gridcolor="#eef2f6",
            gridwidth=1,
            zeroline=False,
        ...
        yaxis=dict(
            title=dict(text="수학 점수", font=_axis_title_font),
            showgrid=True,
            gridcolor="#eef2f6",
            gridwidth=1,
            zeroline=False,
...
def _render_student_report_write_panel(
    ...
    *,
    ...
    """Report builder — pick exams, generate markdown preview and PDF."""
    sid = _parse_grade_student_id(student_id)
    if sid is None:
    ...
    md_state_key = f"{key_prefix}_uni_report_md_{sid}"
    df_state_key = f"{key_prefix}_uni_report_df_{sid}"

    with st.container(border=True):
        st.markdown("#### 보고서에 포함할 시험")
        st.caption("카테고리별로 시험을 선택한 뒤 「제작」을 누르세요.")
    ...
    with st.container(border=True):
        ...
        gen = st.button(
            "제작",
            type="primary",
            key=f"{key_prefix}_uni_report_gen_{sid}",
            use_container_width=True,
        )
        if gen:
            if selected_df.empty:
                st.warning("보고서에 포함할 시험을 하나 이상 선택해 주세요.")
            else:
                st.session_state[md_state_key] = format_unified_grade_report_markdown(
                    student_name, selected_df
                )
                st.session_state[df_state_key] = selected_df

        if st.session_state.get(md_state_key):
            st.markdown(st.session_state[md_state_key])

        report_df = st.session_state.get(df_state_key)
        if (
            report_df is not None
            and not report_df.empty
            and st.session_state.get(md_state_key)
        ):
            try:
                pdf_bytes, pdf_fname = _generate_unified_grade_report_pdf(
                    student_name, report_df
                )
                st.download_button(
                    "PDF 다운로드",
                    data=pdf_bytes,
                    file_name=pdf_fname,
                    mime="application/pdf",
                    key=f"{key_prefix}_uni_pdf_dl_{sid}",
                    use_container_width=True,
                )
            except Exception as exc:
                st.error(f"PDF 생성 실패: {exc}")
...
def _render_student_grade_view_page(teacher_id: int | None) -> None:
    ...
    st.markdown(
        ...
        <div class="grade-dash-hero">
            <p class="grade-dash-hero-label">Ferma Academy</p>
            <h2 class="grade-dash-hero-title">성적 리포트 대시보드</h2>
            <p class="grade-dash-hero-desc">
    ...
    with tab_chart:
        _render_grade_trend_chart(df, chart_key=f"grade_trend_chart_{parsed_sid}")
...
def _render_web_analysis_report_panel(teacher_id: int | None) -> None:
    """웹 HTML 분석 보고서 — 통합보고서 작성 탭 전용."""
    st.divider()
    st.markdown("#### 📄 웹 분석 보고서")
    st.caption("학생과 시험을 선택한 뒤 HTML 분석 보고서를 생성합니다.")

    selected_student_id, selected_student_name, _class_name = _grade_student_selectbox(
        teacher_id,
        key="web_report_student_sel",
    )
    ...
    if selected_student_id:
        exam_options = _list_web_report_exam_options(int(selected_student_id))
        if not exam_options:
            st.info(
                f"**{selected_student_name or '학생'}**의 저장된 학원 TEST 결과가 없습니다. "
                "학원시험 AI분석 탭에서 오답을 저장한 뒤 다시 시도해 주세요."
            )
        else:
            exam_labels = [opt["label"] for opt in exam_options]
            selected_exam_label = st.selectbox(
                "시험 선택",
                exam_labels,
                key=f"web_report_exam_sel_{selected_student_id}",
            )
            for opt in exam_options:
                if opt["label"] == selected_exam_label:
                    selected_test_id = int(opt["test_id"])
                    break
            st.caption(
                f"선택: **{selected_student_name}** · "
                f"**{selected_exam_label}** (test_id {selected_test_id})"
            )
    else:
        st.caption("학생을 선택하면 응시 시험 목록이 표시됩니다.")

    report_ready = bool(selected_student_id and selected_test_id)

    st.subheader("📝 학부모 전달 메시지")
    parent_comment = st.text_area(
        "학부모님께 전할 메시지 (AI가 작성한 내용을 수정하거나 직접 입력하세요)",
        value=(
            "학생이 이번 시험에서 특정 단원에 강점을 보였습니다. "
            "다음 학기에는 이 부분을 더욱 보완하겠습니다."
        ),
        key="web_report_parent_comment",
        height=120,
    )
    ...
    if st.button(
        "📄 웹 분석 보고서 보기",
        type="primary",
        key="web_report_view_btn",
        disabled=not report_ready,
    ):
        selected_student = selected_student_id
        if not selected_student:
            st.warning("학생을 먼저 선택해주세요.")
        elif not selected_test_id:
            st.warning("시험을 먼저 선택해주세요.")
        else:
            try:
                report_data = _build_web_report_data(
                    student_id=int(selected_student),
                    test_id=int(selected_test_id),
                )
                report_data["parent_comment"] = parent_comment
                html_str = generate_html_report(report_data)
            except ValueError as exc:
                st.error(str(exc))
            except Exception as exc:
                st.error(f"웹 보고서 생성 실패: {exc}")
            else:
                st.session_state["web_report_html"] = html_str
                st.session_state["web_report_context"] = {
                    "student_id": int(selected_student),
                    "test_id": int(selected_test_id),
                }
                sname = report_data["student_name"]
                exam_title = report_data["exam_title"]
                st.session_state["web_report_fname"] = (
                    f"웹보고서_{_sanitize_test_filename_part(sname)}_"
                    f"{_sanitize_test_filename_part(exam_title)}.html"
                )
                st.success("웹 분석 보고서가 생성되었습니다.")

    ctx = st.session_state.get("web_report_context") or {}
    if (
        st.session_state.get("web_report_html")
        and ctx.get("student_id") == selected_student_id
        and ctx.get("test_id") == selected_test_id
    ):
        components.html(
            st.session_state["web_report_html"],
            height=1000,
            scrolling=True,
        )
        st.download_button(
            label="⬇️ 웹 분석 보고서 다운로드 (HTML)",
            data=st.session_state["web_report_html"].encode("utf-8"),
            file_name=st.session_state.get("web_report_fname", "web_report.html"),
            mime="text/html",
            key="web_report_download_btn",
        )
...
def page_grade_report() -> None:
    ...
    with tab_report:
        ...
        _render_web_analysis_report_panel(teacher_id)
...
def page_ai_test_analysis() -> None:
    ...
    if ocr_btn:
        if test_upload is None:
            ...
            try:
                ...
                with st.spinner("2/2 GPT-4o 수식 정제 중…"):
                    refined_pages = refine_ocr_pages_with_gpt(raw_ocr["pages"])
                    st.session_state["ocr_extract_result"] = assemble_exam_extraction(
                        raw_ocr,
                        refined_pages,
                    )
                    st.session_state["ocr_parsed_questions"] = (
                        parse_questions_from_extraction(
                            st.session_state["ocr_extract_result"]
                        )
                    )
    ...
    with right_ocr:
        ...
        if "ocr_error" in st.session_state:
            ...
            st.info(
                "왼쪽에서 시험지 파일을 업로드하고 **AI로 시험 분석하기** 버튼을 클릭하면 여기에 리포트가 표시됩니다."
            )
        else:
...
def _nav_teacher_selectbox() -> None:
    ...
    if logged_id:
        ...
        if st.button("로그아웃", key="nav_logout_btn", use_container_width=True):
...
def _nav_teacher_admin() -> None:
    ...
    if not teachers_df.empty:
        ...
        if st.button("강사 삭제", key="nav_t_del_btn", use_container_width=True):
