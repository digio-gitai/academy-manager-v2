...
_PAGE_CHECK_PROMPT = (
    ...
    "활자입니다. 대부분 페이지 맨 아래쪽(하단)에 있지만, 프린트물은 상단이나 "
    "구석에 있을 수도 있으니 페이지 전체 가장자리를 훑어보세요 — 하단 왼쪽/"
    "정중앙/오른쪽을 먼저 보고, 없으면 상단 구석도 확인하세요. "
    "이 '인쇄된 페이지 번호'를 최우선으로 찾아서 답하세요. "
    ...
    "주변에 다른 글자 없이 숫자 하나만 인쇄되어 있어도(그게 흔한 형태입니다), "
    "그 위치·크기가 페이지 번호다운 모양이면 그대로 답하세요 — 확신이 100%가 "
    "아니어도 인쇄된 숫자가 보인다면 '모름'보다는 그 숫자로 답하는 쪽을 "
    "우선하세요(최종 확인은 선생님이 사진을 직접 보고 하므로, 이건 참고용 "
    "1차 판단일 뿐입니다). "
    "정말로 페이지 번호로 보이는 숫자가 사진 어디에도 전혀 없을 때만 '모름'이라고 "
    "답하세요. "
