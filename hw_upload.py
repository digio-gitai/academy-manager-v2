    ...
    try:
        ...
        for it in items:
            ...
            if new_photo_bytes_list:
                # [2026-08-13] material_name + a.class_id도 함께 가져온다 —
                # 참조 PDF 대조 방식(hw_reference.py)이 "이 반 + 이 문제집
                # 이름"으로 등록된 자료를 찾으려면 필요하다. hw_items는
                # class_id를 직접 갖지 않아서(반은 과제 단위로 정해짐)
                # hw_assignments를 조인해서 가져온다.
                item_row = conn.execute(
                    "SELECT i.item_type, i.material_name, i.page_start, i.page_end, a.class_id "
                    "FROM hw_items i JOIN hw_assignments a ON a.id = i.assignment_id "
                    "WHERE i.id = ?",
                    (it["item_id"],),
                ).fetchone()
                item_type = item_row[0] if item_row else None
                item_material_name = item_row[1] if item_row else None
                page_start = item_row[2] if item_row else None
                page_end = item_row[3] if item_row else None
                item_class_id = item_row[4] if item_row else None
            ...
            for photo_bytes in new_photo_bytes_list:
                ...
                if item_type == "page_range" and page_start and page_end:
                    try:
                        ...
                        run_ai_page_check(
                            photo_id, data_uri, int(page_start), int(page_end),
                            class_id=item_class_id, material_name=item_material_name,
                        )
                    except Exception:
                        pass
